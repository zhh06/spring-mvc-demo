...
@Slf4j
@Component(value = "redisUtilsSecond")
public class RedisUtilsSecond<T> {

    private static final String REDIS_KEY_PREFIX = "YNote::%s";

    @Resource
    private RedisTemplate<String, T> redisTemplateSecond;
    ...
    public T get(final String key) {
        ...
        try {
            ValueOperations<String, T> operations = redisTemplateSecond.opsForValue();
    ...
    public Boolean sadd(final String key, Set<T> valueSet) {
        String wholeKey = getKey(key);
        try {
            SetOperations operations = redisTemplateSecond.opsForSet();
    ...
    public Boolean sadd(final String key, T value) {
        String wholeKey = getKey(key);
        try {
            SetOperations<String, T> operations = redisTemplateSecond.opsForSet();
    ...
    public Boolean expire(final String key, Long seconds) {
        String wholeKey = getKey(key);
        try {
            BoundValueOperations boundValueOperations = redisTemplateSecond
    ...
    public void delete(final String key) {
        String wholeKey = getKey(key);
        try {
            redisTemplateSecond.delete(wholeKey);
        } catch (Exception e) {
...
//        return redisTemplateSecond.delete(keys);
//    }
    ...
    public boolean acquireLock(final String key, Long expireTime) {
        ...
        String wholeKey = getKey(key);
        try {
            Object execute = redisTemplateSecond.execute(
                    (RedisCallback) redisConnection -> {
                        RedisSerializer stringSerializer = redisTemplateSecond
                                .getKeySerializer();
    ...
    public boolean checkLock(final String key) {
        String wholeKey = getKey(key);
        try {
            return redisTemplateSecond.hasKey(wholeKey);
        } catch (Exception e) {
